...
def getKmerTruncArea(kmerRanks_D, spots, keep):
    
	kmerTruncAreas_D = {}

	for key in kmerRanks_D:
	  kmerTruncAreas_D[key] = truncEnrichU1Array(kmerRanks_D[key], spots, keep)
    
	return kmerTruncAreas_D
...
def wobbleSeedRerank(kmerRanks_DL, center, PWM_DL, arrayFile, spotLength, startPos, keep):
...
def findMinInfoPos(areaPWM_DL,seed,scaleFactor):


	seedLength = len(seed)

	tempPWM_DL = {i : [0]*seedLength for i in bases}
	minEntropy = 5
	
	if seedLength != len(areaPWM_DL["A"]):
		print "PWM not same size as input seed in findMinInfoPos" 
		sys.exit(0)

	if (len(areaPWM_DL["A"]) != len(areaPWM_DL["C"])) or (len(areaPWM_DL["A"]) != len(areaPWM_DL["G"])) or (len(areaPWM_DL["A"]) != len(areaPWM_DL["T"])):
		print "PWM rows do not have same number of entries in findMinInfoPos" 
		sys.exit(0)
	...
	for i in range(len(seed)):
		...
		if seed[i].upper() in bases:
			...
			for j in bases:
				tempPWM_DL[j][i] = exp(areaPWM_DL[j][i]*scaleFactor)

				denominator += tempPWM_DL[j][i]

			for k in bases:
				tempPWM_DL[k][i] /= float(denominator)
				entropy += tempPWM_DL[k][i] * log(tempPWM_DL[k][i],2) # log to base of 2, since  log x / log 2 = log base2 of x.
				## !!! MIGHT NEED TO CHECK NOT TAKING LOG of 0!! probably won't happen because seed[i]
				## must be in bases! i.e. not "." i.e. not 0 ?
...
def extSeedAllPatternsRr(kmerRanks_DL, center, minInfoPos, PWM_DL, arrayFile,
...
def calculatePWM(seed, kmerRanks_DL, intensityFile, spotLength, startPosition, scaleFactor,totalPatterns_L):
  ...
  seedAreaPWM_DL = {i : [0]*x for i in bases} # Perl can get away without this, but python - as I have used it - needs it. 
  ...
  wobbleSeedRerank(kmerRanks_DL,seed,seedAreaPWM_DL, intensityFile, spotLength, startPosition,1)
  ...
  minInfoPos = findMinInfoPos(seedAreaPWM_DL,seed,scaleFactor)
  ...
  extSeedAllPatternsRr(kmerRanks_DL,seed, minInfoPos, seedAreaPWM_DL, intensityFile, spotLength, startPosition, 1, totalPatterns_L)
  # By reference, this fills up the positions in the PWM not filled in by wobbleSeedRerank

    
  #the following two loops get rid of all leading and trailing "." elements.
  while [seedAreaPWM_DL[k][0] for k in bases] == [0,0,0,0]:
    for l in bases: seedAreaPWM_DL[l].pop(0)

  while [seedAreaPWM_DL[k][-1] for k in bases] == [0,0,0,0]:
    for l in bases: seedAreaPWM_DL[l].pop()
  
  return seedAreaPWM_DL
